...
UserLunchBoxDao.prototype.getListByUserIdAndLunchBoxIdList = function(req, params, callback) {
	// try {
	// 	validator.isDate(params.saleDate);
	// } catch(error) {
	// 	callback(error);
	// 	return;
	// }
	if (params.lunchBoxIdList.length == 0) {
		callback(null, []);
		return
	}
	var queryString = '(' + _.map(params.lunchBoxIdList, function(lunchBoxId) {
        return _.escape(lunchBoxId);
    }).join(', ') + ')';

	mysql.query('select * from user_lunch_box where user_id = ' + params.userId + ' and lunch_box_id in ' + queryString, function (err, rows) {
		if (err) {
			callback(err);
			return;
		}
